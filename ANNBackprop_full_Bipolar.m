function [Recog_rate, benar] = ANNBackprop_full_Bipolar(input, target, n_hidden)
...
for mm = 1:n
    for ha = 1:b
        Norm_in(mm,ha) = ( ( (input_train(mm,ha) - min(input_train(:,ha)))*(1+1) )/( max(input_train(:,ha)) - min(input_train(:,ha)) ) )-1;
    end
end
for mm = 1:m
    for ha = 1:a
        Norm_tr(mm,ha) = ( ( (target_train(mm,ha) - min(target_train(:,ha)))*(1+1) )/( max(target_train(:,ha)) - min(target_train(:,ha)) ) )-1;
    end
end
input_train = Norm_in;
...
while kondisi == 0 && epoch ~= max_epoch+1
    ...
    for j = 1:p
        Z(1,j) = ( exp(Z_in(1,j)) - exp( -Z_in(1,j)) )./( exp(Z_in(1,j)) + exp( -Z_in(1,j)) );
    end
    ...
    for k = 1:a
        Y(1,k) = ( exp(Y_in(1,k)) - exp( -Y_in(1,k)) )./( exp(Y_in(1,k)) + exp( -Y_in(1,k)) );
    end
    ...
    for j = 1:p
        doj(1,j) = do_in(1,j) .* (1 - (Z(1,j)).^2 );
    end
    
    for i = 1:b
        for j = 1:p
            deltaV(i,j) = alpha.*doj(1,j).*X(i) + MOMV(i,j);
            deltaV0(1,j) = alpha.*doj(1,j);
        end
    end
...
for mm = 1:n
    for ha = 1:b
        Norm_in1(mm,ha) = ( ( (input_test(mm,ha) - min(input_test(:,ha)))*(1+1) )/( max(input_test(:,ha)) - min(input_test(:,ha)) ) )-1;
    end
end
for mm = 1:m
    for ha = 1:a
        Norm_tr1(mm,ha) = ( ( (target_test(mm,ha) - min(target_test(:,ha)))*(1+1) )/( max(target_test(:,ha)) - min(target_test(:,ha)) ) )-1;
    end
end
input_test = Norm_in1;
